...
- **Drift** is drawn fresh per trial from `N(drift_mean, drift_sigma)`.
  When `fix_drift = true`, `drift_sigma = 0`, drift is constant (MS).
...
All scripts are in `scripts/` and resolve paths relative to their own
location (`data/`, `results/`, `figures/` are created/read as siblings).

| File | Purpose |
|------|---------|
| `diffusion_model_fig4.m` | Forward simulation + the manuscript figure set. Runs the model with fixed parameters, loads monkey data, and produces the demo traces, response-frequency, and RT figures. Includes the session-level RT trend/asymmetry tests (Wilcoxon signed-rank). |
...
`diffusion_model_fig4.m` uses hard-coded parameters (from the
...
- **Data** in `data/`: `bhv_data_tbl_m1.mat` and `bhv_data_tbl_m2.mat`,
  each holding a `dataTable` with per-session `RespMat` (trial data and produced counts),
  `RT_conf`, and `RT_err_conf`. 
